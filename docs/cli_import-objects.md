...
```
  cli import-objects --team-id 863ea5b9-7ed4-43bb-82aa-bc5508ef8716 --object-type document --objects '[{"list":{}}]'
```
...
* `cli import-objects --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli import-objects --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli import-objects --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
