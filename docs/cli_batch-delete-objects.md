...
```
cli batch-delete-objects [flags]
```

### Examples

```
  cli batch-delete-objects --team-id 95aef178-55b0-4a28-a504-e7907ae2bb78 --object-type action --ids <value>
```
...
```
  ...
  -h, --help                                       help for batch-delete-objects
...
* `cli batch-delete-objects --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli batch-delete-objects --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli batch-delete-objects --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
