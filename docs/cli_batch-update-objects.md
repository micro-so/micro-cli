...
```
  cli batch-update-objects --team-id 8b4a4ef5-5244-4c0c-b879-78d0edf7068d --object-type ai_chat_message --items '[]'
```
...
* `cli batch-update-objects --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli batch-update-objects --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli batch-update-objects --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
