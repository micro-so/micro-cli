...
```
  cli patch-metadata-property-option --team-id e8e78353-372c-4662-9b0e-ce25f7aa085d --object-type deal --property-id 2e0aa9c0-3a10-452e-acf5-5d988bc5985a --option-id f7c352cb-18ee-453a-92a4-94a9bc41a045 --type multiref_attendee
```
...
* `cli patch-metadata-property-option --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli patch-metadata-property-option --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli patch-metadata-property-option --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
