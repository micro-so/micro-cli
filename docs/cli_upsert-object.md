...
```
  cli upsert-object --team-id 1fa0e3b3-bd09-49e7-99c2-f37eb2a55a73 --object-type ai_chat_message --slug <value> --value <value> --body '{"list":{}}'
```
...
* `cli upsert-object --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli upsert-object --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli upsert-object --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
