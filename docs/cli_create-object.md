...
```
  cli create-object --team-id 74f8c1fb-f129-4875-899e-8f92fabd0e94 --object-type ai_chat_thread --body '{"default":{"key":"<value>","key1":"<value>"}}'
```
...
* `cli create-object --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli create-object --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli create-object --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
