...
```
  cli patch-metadata-property --team-id 0b096073-a49b-49b5-8a13-ac1c438aa436 --object-type event --property-id 9563eedd-e652-46fb-86a8-454fb3549b69 --type multiref_account
```
...
* `cli patch-metadata-property --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli patch-metadata-property --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli patch-metadata-property --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
